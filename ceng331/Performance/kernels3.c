                    ...
                    for(i = 0; i < dim; i++){
                       for(j = 0; j < dim; j++){
                            for(i1=i; i1<i+3; i1++){
                              for(j1=j; j1<j+3; j1++){
                                dst[j*dim+i]=0;
                                if(!((i1 == 0) || (i1 == dm-1) || (j1 == 0) || (j1 == dm-1))){
                                  dst[j1*dim+i1]=dst[j1*dim+i1]+src[(j1 - 1)*dim+(i1 - 1)] * ker[0][0];
                                  dst[j1*dim+i1]=dst[j1*dim+i1]+src[(j1)*dim+(i1 - 1)] * ker[1][0];

                                  dst[j1*dim+i1]=dst[j1*dim+i1]+src[(j1 - 1)*dim+(i1)] * ker[0][1];
                                  dst[j1*dim+i1]=dst[j1*dim+i1]+src[(j1)*dim+(i1)] * ker[1][1];
                                 }
                              }
                            }
                       }
