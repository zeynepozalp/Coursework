...
private: // you may add your own utility member functions here.
    ...
    void removeSN(PrimaryNode* pn, SecondaryNode* &snRoot, SecondaryNode* &sn){
      ...
      if(sn->key<snRoot->key) removeSN(pn,snRoot->left, sn);
        ...
        SecondaryNode* parent=findParent(pn->rootSecondaryNode, snRoot, i);
        SecondaryNode* newNode= new SecondaryNode(max->key, max->data, NULL, NULL);
        newNode->left=snRoot->left;
        newNode->right=snRoot->right;
        if(i==-1){
          pn->rootSecondaryNode=newNode;
        }
        else if(i==0){
          parent->left=newNode;
        }
        ...
        if(max->left!=NULL){
          insertSN(pn, pn->rootSecondaryNode, max->left->key, max->left->data);
        }
        deleteMax(newNode->left);
        delete sn;
      }
      else{
